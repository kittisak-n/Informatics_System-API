...
(exports.Get_person_by_id = (req, res) => {
  ...
  (exports.get_summary_by_year = (req, res) => {
    ...
    sql_get_summary_by_year +=
      "LEFT JOIN wls_schedule ON wls_summary.schedule_id = wls_schedule.schedule_id\n";
    sql_get_summary_by_year +=
      "RIGHT JOIN pms_person ON wls_summary.person_id = pms_person.person_id AND wls_summary.summary_year = ? AND wls_summary.summary_term = ? \n";
    sql_get_summary_by_year +=
      "LEFT JOIN pms_position ON pms_person.person_position = pms_position.position_id\n";
    sql_get_summary_by_year +=
      "LEFT JOIN pms_prefix ON pms_person.prefix_id = pms_prefix.pf_id\n";
    ...
    sql_get_summary_by_year +=
      "LEFT JOIN ifs_districts ON pms_person.person_district = ifs_districts.id\n";
      sql_get_summary_by_year += "WHERE pms_person.person_position != 1 "
    sql_get_summary_by_year += "GROUP BY pms_person.person_id \n";
  ...
  (exports.get_summary_by_person_id = (req, res) => {
    ...
    sql_get_summary_by_person_id +=
      "LEFT JOIN ifs_districts ON pms_person.person_district = ifs_districts.id\n";
    sql_get_summary_by_person_id += "WHERE wls_summary.person_id = ? AND wls_summary.summary_year = ? AND wls_summary.summary_term = ? \n";
    ...
    try {
      dbConnect.query(
        sql_get_summary_by_person_id,
        [req.body.person_id,req.body.summary_year,req.body.summary_term],
        (err, results) => {
          if (err) {
            res.json({
              status: false,
...
exports.get_summary_of_report = (req, res) => {
  ...
  sql_get_summary_of_report += "SELECT *\nFROM wls_summary_of_report\n";
  sql_get_summary_of_report += "WHERE summary_id = ?";
