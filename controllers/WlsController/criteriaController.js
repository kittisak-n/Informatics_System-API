...
exports.Add_schedule = (req, res) => {
  ...
  try {
    if(  req.body.schedule_status == 0){
      console.log("วันที่สร้างแล้ววันที่ปัจจุบันไม่ตรงกัน ให้ใช้ตัวเก่า");
          console.log(req.body.schedule_status);
    }
    else{
      console.log("วันที่สร้างแล้ววันที่ปัจจุบันตรงกัน ให้ใช้ตัวที่สร้างใหม่");
      Update_status_schedule( function (results){
        console.log(results);
      });
    }
    dbConnect.query(
...
exports.Get_all_schedule = (req, res) => {
  let sql_get_schedule =
    "SELECT schedule_id,SUBSTRING(schedule_start_date,1,10) AS schedule_start_date,schedule_name,schedule_general_min,schedule_general_max,schedule_per_credit,schedule_status,schedule_create_by,schedule_create_date  FROM wls_schedule ORDER BY schedule_start_date DESC ";
...
const Update_status_schedule = (callback) => {
